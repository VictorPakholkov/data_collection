...
Другой проблемой с note_1 и note_2 оказался несогласованный в ТЗ формат разметки, который разметчики оставляли, в случае если во всех других полях были нули. Первый разметчик всегда просто перечислял факторы из ревью (cast, plot, atmosphere), в то время как второй разметчик добавлял эпитеты, испольуемые автором (good cast, gripping plot, cozy atmosphere).
...
У первого разметчика так же есть одно пропцщенное наблюдение, я решил удалить эту строку из датасета полностью.
